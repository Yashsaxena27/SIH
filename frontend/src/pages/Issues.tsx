  ...
  return (
    <div className={cn("flex flex-col space-y-4 sm:space-y-6", viewMode === 'map' ? "h-[calc(100vh-3.5rem)] p-0" : "p-4 sm:p-6")}>
      ...
      <div className={cn("flex flex-col lg:flex-row lg:items-end justify-between gap-4", viewMode === 'map' && "absolute top-4 left-4 right-4 z-10")}>
        ...
        <div className={cn(viewMode === 'map' && "bg-black/40 backdrop-blur-md p-4 rounded-xl border border-black/[0.1] w-max")}>
          <h1 className="text-2xl font-bold text-black/95 tracking-tight flex items-center gap-3">
            Urban Issues
            <span className="px-2.5 py-0.5 rounded text-[11px] font-bold bg-black/[0.08] text-black/70 border border-black/[0.1]">
              {filteredIssues.length}
            </span>
          </h1>
          <p className="text-sm text-black/40 mt-1">Detected and prioritized infrastructure events</p>
        </div>

        <div className={cn("flex flex-col sm:flex-row items-center gap-3", viewMode === 'map' && "bg-black/40 backdrop-blur-md p-2 rounded-xl border border-black/[0.1]")}>
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-black/[0.03] border border-black/[0.08] rounded-lg text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-accent-primary/50 transition-colors"
            />
          </div>

          <button className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-black/[0.03] hover:bg-black/[0.06] border border-black/[0.08] rounded-lg text-sm font-medium text-black/80 transition-colors">
            <Filter className="w-4 h-4" /> Filters
          </button>

          {/* View Toggles */}
          <div className="flex items-center p-1 bg-black/[0.03] border border-black/[0.08] rounded-lg">
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  viewMode === mode.id ? "bg-black/[0.1] text-black shadow-sm" : "text-black/40 hover:text-black/80"
                )}
              >
                ...
                <GlassPanel hover padding="md" className="cursor-pointer group flex flex-col h-full relative overflow-hidden">
                  ...
                  <div className={cn("flex justify-between gap-4 relative z-10", viewMode === 'list' ? "items-center" : "items-start flex-col sm:flex-row")}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-lg font-bold text-black/90 truncate capitalize mb-1 group-hover:text-black transition-colors">
                    ...
                    {/* Stats */}
                    <div className={cn("flex items-center gap-4 shrink-0", viewMode === 'grid' && "w-full justify-between mt-2 pt-4 border-t border-black/[0.04]")}>
                      
                      <div className="flex flex-col items-center sm:items-end">
                        <div className="text-[10px] font-semibold text-black/30 uppercase tracking-widest mb-1">Observations</div>
                        <div className="flex items-center gap-1.5">
                          <Bus className="w-3.5 h-3.5 text-black/40" />
                          <span className="text-sm font-bold text-black/80">{issue.observationCount}</span>
                        </div>
                      </div>

                      <div className="flex flex-col items-center sm:items-end">
                        <div className="text-[10px] font-semibold text-black/30 uppercase tracking-widest mb-1">Confidence</div>
                        <div className="flex items-center gap-1.5">
                          <ShieldCheck className="w-3.5 h-3.5 text-accent-primary" />
                          <span className="text-sm font-bold text-black/80">{(issue.confidence * 100).toFixed(0)}%</span>
                        </div>
                      </div>
                  ...
                  {/* Footer Line */}
                  <div className="mt-4 pt-3 border-t border-black/[0.04] flex items-center justify-between text-[11px] relative z-10">
