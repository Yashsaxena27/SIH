      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div>
          <h3 className="text-sm font-bold text-on-surface uppercase tracking-widest flex items-center gap-2">
            <Zap className="w-4 h-4 text-secondary" /> Live Sensor Pipeline
          </h3>
          <p className="text-[11px] text-on-surface-variant mt-1">Data architecture from physical edge to spatial database.</p>
        </div>
        <div className="px-2 py-0.5 rounded border border-secondary/30 bg-secondary/10 text-secondary text-[10px] font-bold uppercase animate-pulse">
          Live Stream
        </div>
      </div>
      ...
      <div className="relative flex justify-between items-center py-4 px-2 sm:px-6 z-10">
        ...
        <div className="absolute left-[10%] right-[10%] top-1/2 -translate-y-1/2 pointer-events-none overflow-hidden h-4">
          {[0, 1, 2].map((i) => (
            <motion.div
              key={i}
              className="absolute w-2 h-2 rounded-full bg-secondary shadow-[0_0_8px_rgba(6,182,212,0.8)] top-1"
...
export function FleetPage() {
  ...
  const loadData = () => {
    ...
    Promise.allSettled([api.getBuses(), api.getRoutes()]).then(([bRes, rRes]) => {
      ...
      if (b.length > 0) {
        b[0] = { ...b[0], status: 'offline', id: 'BUS-042 (Edge Demo)' };
      }
      setBuses(b);
      setRoutes(r);
      setLoading(false);
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1920px] mx-auto h-[calc(100vh-var(--spacing-header-height))] flex flex-col relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-1 font-medium">Real-time health and coverage of the urban sensing network.</p>
        </div>
        ...
        <div className="flex gap-2 sm:gap-4 overflow-x-auto pb-2 sm:pb-0 scrollbar-none">
          {[
            ...
            { label: 'Obs. Today', value: totalObs, color: 'text-status-healthy' },
          ].map(stat => (
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-12 gap-6 overflow-y-auto scrollbar-none pb-10">
        ...
        <div className="xl:col-span-7 space-y-4">
          <h3 className="text-xs font-bold text-on-surface uppercase tracking-widest px-1">Network Nodes</h3>
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <div className="text-[10px] uppercase tracking-widest text-on-surface-variant">Route {bus.routeId || 'Unassigned'}</div>
                    </div>
                    <div className={cn(
                      "px-2 py-0.5 rounded font-label-caps text-[10px] flex items-center gap-1.5",
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                    <div className="flex items-center gap-2 text-xs">
                      <Signal className={cn("w-3.5 h-3.5", isOffline ? "text-red-400" : "text-status-healthy")} />
                      <span className="text-on-surface">{isOffline ? 'Disconnected' : 'Connected'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className="text-on-surface font-data-mono">AI: RUNNING</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <Activity className="w-3.5 h-3.5 text-purple-400" />
                      <span className="text-on-surface">{bus.detectionsToday} Obs.</span>
                    </div>
                  </div>
                  
                  <div className="mt-4 pt-3 border-t border-white/[0.04] flex items-center justify-between text-[10px] text-on-surface-variant font-data-mono">
        ...
        <div className="xl:col-span-5 flex flex-col gap-6">
          ...
          <GlassPanel padding="none" className="flex-1 min-h-[300px] flex flex-col overflow-hidden relative">
            <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between pointer-events-none">
              <div className="bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-outline-variant pointer-events-auto">
              ...
              <div className="flex flex-col gap-1 items-end pointer-events-auto">
                <div className="bg-black/60 backdrop-blur-md px-2 py-1 rounded border border-outline-variant font-label-caps text-[10px] text-purple-400 flex items-center gap-1.5">
                  <div className="w-3 h-0.5 bg-purple-500" /> Observed
                </div>
                <div className="bg-black/60 backdrop-blur-md px-2 py-1 rounded border border-outline-variant font-label-caps text-[10px] text-on-surface-variant flex items-center gap-1.5">
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-outline-variant flex items-start justify-between bg-white/[0.02]">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className={cn(
                      "px-2 py-0.5 rounded font-label-caps text-[10px]",
                  ...
                  <h2 className="text-2xl font-bold text-on-surface">Route {selectedBus.routeId || 'Unassigned'} Node</h2>
                </div>
              ...
              <div className="flex-1 overflow-y-auto scrollbar-none p-6 space-y-8">
                ...
                {selectedBus.status === 'offline' ? (
                  <div className="space-y-4">
                    ...
                    <div className="rounded-xl border border-outline-variant bg-background overflow-hidden">
                      <div className="grid grid-cols-3 divide-x divide-white/[0.08] border-b border-outline-variant">
                        
                        <div className="p-4 flex flex-col items-center justify-center text-center">
                          <Cpu className="w-5 h-5 text-secondary mb-2" />
                          <span className="text-[9px] uppercase font-bold text-on-surface-variant tracking-widest mb-1">Edge AI</span>
                          <span className="text-xs font-bold text-on-surface font-data-mono">RUNNING</span>
                        </div>
                        ...
                        <div className="p-4 flex flex-col items-center justify-center text-center relative overflow-hidden">
                          ...
                          <span className="text-[9px] uppercase font-bold text-on-surface-variant tracking-widest mb-1">Local Buffer</span>
                        ...
                        <div className="p-4 flex flex-col items-center justify-center text-center relative overflow-hidden">
                          <motion.div animate={{ color: simState === 0 ? 'var(--color-status-critical)' : '#10b981' }} className="mb-2">
                            {simState === 0 ? <WifiOff className="w-5 h-5" /> : <Wifi className="w-5 h-5" />}
                          </motion.div>
                          <span className="text-[9px] uppercase font-bold text-on-surface-variant tracking-widest mb-1">Network</span>
                          <span className="text-xs font-bold text-on-surface font-data-mono">
                      ...
                      <div className="p-4 bg-white/[0.02]">
                        {simState === 0 && (
                          <button 
                            ...
                            className="w-full py-2 bg-status-healthy/10 hover:bg-status-healthy/20 border border-status-healthy/30 text-status-healthy font-label-caps rounded-lg transition-colors flex items-center justify-center gap-2"
                          >
                            <RefreshCw className="w-3 h-3" /> Simulate Network Restore
                          </button>
                        )}
                        {simState === 1 && (
                          <div className="w-full py-2 text-center text-status-healthy font-label-caps flex items-center justify-center gap-2">
                            Establishing Connection...
                          </div>
                        )}
                        {simState === 2 && (
                          <div className="w-full py-2 text-center text-purple-400 font-label-caps flex items-center justify-center gap-2">
                            <RefreshCw className="w-3 h-3 animate-spin" /> Uploading buffered events
                          </div>
                        )}
                        {simState === 3 && (
                          <div className="w-full py-2 text-center text-on-surface-variant font-label-caps flex items-center justify-center gap-2">
                            <CheckCircle className="w-3 h-3 text-status-healthy" /> Buffer Synced
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-xl border border-outline-variant bg-white/[0.02]">
                      <div className="font-label-caps text-[10px] text-on-surface-variant mb-1">Speed</div>
                      <div className="text-xl font-bold font-data-mono text-on-surface">24 <span className="text-sm text-on-surface-variant">km/h</span></div>
                    </div>
                    <div className="p-4 rounded-xl border border-outline-variant bg-white/[0.02]">
                      <div className="font-label-caps text-[10px] text-on-surface-variant mb-1">GPS Latency</div>
                      <div className="text-xl font-bold font-data-mono text-on-surface">1.2 <span className="text-sm text-on-surface-variant">s</span></div>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-bold text-on-surface uppercase tracking-widest">Local Node Events</h3>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={evt.id} className="p-3 rounded-lg border border-white/[0.04] bg-white/[0.01] flex items-center justify-between group hover:bg-surface-container transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="text-xs font-data-mono text-on-surface-variant bg-black/40 px-1.5 py-0.5 rounded">{evt.time}</div>
                          <div className="text-sm font-medium text-on-surface">{evt.type}</div>
                        </div>
                        <div className="text-[10px] text-secondary font-data-mono">{evt.conf}</div>
                      </div>
                    ))}
                  </div>
                </div>
