...
function SensorPipeline() {
  const nodes = [
    ...
    { label: 'PostGIS DB', icon: Database, color: 'text-black', bg: 'bg-black' },
  ];
    ...
    <GlassPanel className="relative overflow-hidden mt-6">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div>
          ...
          <p className="text-[11px] text-black/40 mt-1">Data architecture from physical edge to spatial database.</p>
        </div>
      ...
      <div className="relative flex justify-between items-center py-4 px-2 sm:px-6 z-10">
        {/* Track Line */}
        <div className="absolute left-[10%] right-[10%] top-1/2 h-0.5 bg-black/[0.05] -translate-y-1/2" />
        ...
        {nodes.map((node, i) => (
          <div key={node.label} className="relative flex flex-col items-center gap-3 z-10 bg-[#09090b] px-2">
            <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center border border-black/[0.1] bg-surface-raised shadow-xl", node.color)}>
              <node.icon className="w-5 h-5" />
            </div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-black/50">{node.label}</span>
          </div>
        ))}
      </div>
    </GlassPanel>
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-[1920px] mx-auto h-[calc(100vh-3.5rem)] flex flex-col relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 flex-shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-black/95 tracking-tight">
            Mobile Sensing Fleet
          </h1>
          <p className="text-sm text-black/40 mt-1 font-medium">Real-time health and coverage of the urban sensing network.</p>
        </div>
        
        <div className="flex gap-2 sm:gap-4 overflow-x-auto pb-2 sm:pb-0 scrollbar-none">
          {[
            { label: 'Total Fleet', value: buses.length, color: 'text-black/90' },
            ...
            <div key={stat.label} className="bg-black/[0.03] border border-black/[0.06] rounded-lg px-4 py-2 flex flex-col min-w-[100px]">
              <span className="text-[10px] uppercase font-bold tracking-widest text-black/40">{stat.label}</span>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-12 gap-6 overflow-y-auto scrollbar-none pb-10">
        ...
        <div className="xl:col-span-7 space-y-4">
          <h3 className="text-xs font-bold text-black/70 uppercase tracking-widest px-1">Network Nodes</h3>
                ...
                <motion.div
                  ...
                  className={cn(
                    ...
                    isOffline 
                      ...
                      : "bg-black/[0.02] border-black/[0.06] hover:bg-black/[0.08]"
                  )}
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-mono font-bold text-black/90">{bus.id}</span>
                      </div>
                      <div className="text-[10px] uppercase tracking-widest text-black/40">Route {bus.routeId || 'Unassigned'}</div>
                    </div>
                    <div className={cn(
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className="text-black/70">{isOffline ? 'Disconnected' : 'Connected'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <Camera className="w-3.5 h-3.5 text-emerald-400" />
                      <span className="text-black/70">Healthy</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <Cpu className="w-3.5 h-3.5 text-accent-secondary" />
                      <span className="text-black/70 font-mono">AI: RUNNING</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <Activity className="w-3.5 h-3.5 text-purple-400" />
                      <span className="text-black/70">{bus.detectionsToday} Obs.</span>
                    </div>
                  </div>
                  
                  <div className="mt-4 pt-3 border-t border-black/[0.04] flex items-center justify-between text-[10px] text-black/40 font-mono">
        ...
        <div className="xl:col-span-5 flex flex-col gap-6">
          ...
          <GlassPanel padding="none" className="flex-1 min-h-[300px] flex flex-col overflow-hidden relative">
            <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between pointer-events-none">
              <div className="bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-black/[0.1] pointer-events-auto">
                <h3 className="text-xs font-bold text-black/90 uppercase tracking-widest flex items-center gap-2">
                  <MapIcon className="w-4 h-4 text-purple-400" /> Route Coverage
                </h3>
              </div>
              <div className="flex flex-col gap-1 items-end pointer-events-auto">
                <div className="bg-black/60 backdrop-blur-md px-2 py-1 rounded border border-black/[0.1] text-[10px] font-bold uppercase tracking-widest text-purple-400 flex items-center gap-1.5">
                  <div className="w-3 h-0.5 bg-purple-500" /> Observed
                </div>
                <div className="bg-black/60 backdrop-blur-md px-2 py-1 rounded border border-black/[0.1] text-[10px] font-bold uppercase tracking-widest text-black/40 flex items-center gap-1.5">
                  <div className="w-3 h-0.5 bg-black/30 border-t border-dashed" /> Unobserved
                </div>
              </div>
            </div>
        ...
        {selectedBus && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 top-0 bottom-0 w-full sm:w-[480px] z-[500] bg-[#0f0f12] border-l border-black/[0.08] shadow-2xl flex flex-col"
            >
              <div className="p-6 border-b border-black/[0.06] flex items-start justify-between bg-black/[0.02]">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] text-black/40 font-mono tracking-widest">{selectedBus.id}</span>
                  </div>
                  <h2 className="text-2xl font-bold text-black/95">Route {selectedBus.routeId || 'Unassigned'} Node</h2>
                </div>
                <button onClick={() => setSelectedBus(null)} className="p-2 text-black/40 hover:text-black rounded-lg hover:bg-black/[0.05]">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto scrollbar-none p-6 space-y-8">
                ...
                {selectedBus.status === 'offline' ? (
                  <div className="space-y-4">
                    <h3 className="text-xs font-bold text-black/70 uppercase tracking-widest flex items-center gap-2">
                      <HardDrive className="w-4 h-4 text-purple-400" /> Offline-First Architecture
                    </h3>
                    
                    <div className="rounded-xl border border-black/[0.08] bg-[#09090b] overflow-hidden">
                      <div className="grid grid-cols-3 divide-x divide-black/[0.08] border-b border-black/[0.08]">
                        
                        <div className="p-4 flex flex-col items-center justify-center text-center">
                          <Cpu className="w-5 h-5 text-accent-secondary mb-2" />
                          <span className="text-[9px] uppercase font-bold text-black/40 tracking-widest mb-1">Edge AI</span>
                          <span className="text-xs font-bold text-black/90 font-mono">RUNNING</span>
                        </div>
                        ...
                        <div className="p-4 flex flex-col items-center justify-center text-center relative overflow-hidden">
                          ...
                          <span className="text-[9px] uppercase font-bold text-black/40 tracking-widest mb-1">Local Buffer</span>
                          <span className="text-xs font-bold text-black/90 font-mono">
                        ...
                        <div className="p-4 flex flex-col items-center justify-center text-center relative overflow-hidden">
                          ...
                          <span className="text-[9px] uppercase font-bold text-black/40 tracking-widest mb-1">Network</span>
                          <span className="text-xs font-bold text-black/90 font-mono">
                      ...
                      <div className="p-4 bg-black/[0.02]">
                        ...
                        {simState === 3 && (
                          <div className="w-full py-2 text-center text-black/50 text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-xl border border-black/[0.06] bg-black/[0.02]">
                      <div className="text-[10px] uppercase font-bold tracking-widest text-black/40 mb-1">Speed</div>
                      <div className="text-xl font-bold font-mono text-black/90">24 <span className="text-sm text-black/40">km/h</span></div>
                    </div>
                    <div className="p-4 rounded-xl border border-black/[0.06] bg-black/[0.02]">
                      <div className="text-[10px] uppercase font-bold tracking-widest text-black/40 mb-1">GPS Latency</div>
                      <div className="text-xl font-bold font-mono text-black/90">1.2 <span className="text-sm text-black/40">s</span></div>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-bold text-black/70 uppercase tracking-widest">Local Node Events</h3>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={evt.id} className="p-3 rounded-lg border border-black/[0.04] bg-black/[0.01] flex items-center justify-between group hover:bg-black/[0.03] transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="text-xs font-mono text-black/40 bg-black/40 px-1.5 py-0.5 rounded">{evt.time}</div>
                          <div className="text-sm font-medium text-black/80">{evt.type}</div>
                        </div>
