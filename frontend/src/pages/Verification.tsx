      ...
      {type === 'after_success' && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative w-36 h-28">
            ...
            <div className="absolute inset-0 bg-[#2a2a35] rounded-[35%_65%_65%_35%/45%_55%_55%_45%] border border-black/5" />
            {/* Bounding Box */}
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-[1920px] mx-auto h-[calc(100vh-3.5rem)] flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-black/40 mt-1 font-medium">Closing the loop between detection and resolution.</p>
        </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-6">
        ...
        <GlassPanel className="w-full lg:w-80 flex flex-col overflow-hidden flex-shrink-0">
          <div className="p-4 border-b border-black/[0.06] flex items-center justify-between">
            <h3 className="text-xs font-bold text-black/70 uppercase tracking-widest">Verification Queue</h3>
            <span className="text-[10px] text-black/40 bg-black/[0.05] px-2 py-0.5 rounded-full">{mockCases.length} cases</span>
          </div>
            ...
            {mockCases.map(c => (
              <button
                ...
                className={cn(
                  ...
                  activeCase.id === c.id 
                    ? "bg-black/[0.08] border-black/[0.15] shadow-lg" 
                    : "bg-black/[0.02] border-black/[0.04] hover:bg-black/[0.05]"
                )}
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs font-mono text-black/50">{c.id}</span>
                  <span className={cn(
                ...
                <div className="text-sm font-bold text-black/90 mb-0.5">{c.type}</div>
                <div className="text-xs text-black/40 truncate">{c.location}</div>
              </button>
            ))}
          </div>
        </GlassPanel>
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-6 overflow-y-auto scrollbar-none pb-10 lg:pb-0">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <GlassPanel className="xl:col-span-2 overflow-hidden flex flex-col p-0 border-black/[0.1]">
              <div className="px-4 py-3 border-b border-black/[0.06] flex items-center justify-between bg-surface-raised/50">
                <div className="flex items-center gap-2">
                  <Cpu className="w-4 h-4 text-accent-primary" />
                  <span className="text-xs font-bold text-black/80 uppercase tracking-widest">Visual Comparison</span>
                </div>
                <div className="text-[10px] font-mono text-black/30">{activeCase.id}</div>
              </div>
              ...
              >
                ...
                <div className="absolute inset-0">
                  ...
                  <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-black/[0.1] z-10">
                    <div className="text-[10px] text-black/40 uppercase font-bold tracking-widest mb-0.5">Original Detection</div>
                    <div className="text-xs text-black/80 font-mono">{formatDate(activeCase.beforeTime, 'medium')}</div>
                  </div>
                </div>
                ...
                >
                  ...
                  {activeCase.status !== 'pending' && (
                    <div className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-black/[0.1] text-right z-10">
                      <div className="text-[10px] text-black/40 uppercase font-bold tracking-widest mb-0.5">Bus Reinspection</div>
                      <div className="text-xs text-black/80 font-mono">{formatDate(activeCase.afterTime, 'medium')}</div>
                    </div>
                  )}
                </div>
            ...
            <GlassPanel className="xl:col-span-1 flex flex-col border-accent-primary/20 bg-accent-primary/[0.02] overflow-hidden">
              <div className="px-4 py-3 border-b border-accent-primary/[0.1] flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-black/80 uppercase tracking-widest">AI Analysis</span>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-center space-y-6">
                  ...
                  <AnimatePresence mode="wait">
                    {analysisState === 0 && (
                      <motion.div key="state0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-3">
                        <div className="w-5 h-5 border-2 border-black/20 border-t-accent-primary rounded-full animate-spin" />
                        <span className="text-sm font-medium text-black/50">Aligning spatial frames...</span>
                      </motion.div>
                    )}
                    {analysisState === 1 && (
                    ...
                    {analysisState === 2 && (
                      <motion.div key="state2" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full">
                        <div className={cn(
                          ...
                          activeCase.status === 'reopened' ? "bg-red-500/10 border-red-500/30" : "bg-black/[0.05] border-black/[0.1]"
                        )}>
                          {activeCase.status === 'verified' ? (
                            <>
                              ...
                              <div className="text-xs text-black/60 mt-1">Defect geometry no longer present</div>
                            </>
                            ...
                            <>
                              ...
                              <div className="text-xs text-black/60 mt-1">Awaiting bus route coverage</div>
                            </>
                          )}
                        </div>
                      </motion.div>
                ...
                <div className="space-y-4 pt-4 border-t border-black/[0.06]">
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-black/40 uppercase tracking-wider font-semibold">Repair Confidence</span>
                    <span className="text-lg font-bold text-black/90">
                      {analysisState === 2 ? `${activeCase.confidence}%` : '---'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-black/40 uppercase tracking-wider font-semibold">Road Surface</span>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <GlassPanel padding="md">
              <h3 className="text-xs font-bold text-black/70 uppercase tracking-widest mb-6 flex items-center gap-2">
              ...
              <div className="flex items-center gap-4 relative">
                ...
                {/* Original Bus */}
                <div className="flex-1 bg-black/[0.02] border border-black/[0.06] rounded-xl p-4 relative z-10">
                  <div className="text-[10px] text-black/30 uppercase tracking-widest mb-2 font-bold">Original Detection</div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black/90 font-mono">{activeCase.beforeBus}</div>
                      <div className="text-xs text-black/40">{formatDate(activeCase.beforeTime, 'short')}</div>
                    </div>
                  </div>
                </div>

                {/* Connection */}
                <div className="flex-shrink-0 flex flex-col items-center justify-center z-10 text-black/20">
                ...
                <div className="absolute left-1/4 right-1/4 top-1/2 h-px bg-black/[0.05] z-0 border-dashed border-t border-black/[0.1]" />

                {/* Reinspection Bus */}
                <div className={cn(
                  "flex-1 border rounded-xl p-4 relative z-10 transition-colors",
                  activeCase.status !== 'pending' ? "bg-black/[0.02] border-black/[0.06]" : "bg-transparent border-dashed border-black/[0.1]"
                )}>
                  <div className="text-[10px] text-black/30 uppercase tracking-widest mb-2 font-bold">Reinspection</div>
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      "w-10 h-10 rounded-full flex items-center justify-center border",
                      activeCase.status !== 'pending' ? "bg-accent-secondary/10 border-accent-secondary/20 text-accent-secondary" : "bg-black/[0.02] border-black/[0.1] text-black/20"
                    )}>
                      <Bus className="w-4 h-4" />
                    </div>
                    <div>
                      <div className={cn("text-sm font-bold font-mono", activeCase.status !== 'pending' ? "text-black/90" : "text-black/30")}>
                        {activeCase.afterBus}
                      </div>
              ...
              <div className="mt-4 px-4 py-3 bg-black/[0.03] rounded-lg border border-black/[0.05] text-xs text-black/50 text-center">
            ...
            {/* Vertical Timeline */}
            <GlassPanel padding="md">
              <h3 className="text-xs font-bold text-black/70 uppercase tracking-widest mb-6">Workflow Timeline</h3>
              ...
              <div className="relative pl-4 space-y-5">
                ...
                {[
                  ...
                  { label: 'Ticket Created', active: true, icon: PenTool, color: 'text-black/60' },
