...
export function Timeline({ events, className }: TimelineProps) {
  return (
    <div className={cn('relative pl-4', className)}>
      <div className="absolute left-4 top-2 bottom-2 w-px bg-white/[0.1]" />
          ...
          return (
            <div key={event.id} className="relative pl-8">
              <div className={cn(
                ...
                event.color || 'text-white/56'
              )}>
                {Icon ? <Icon className="h-3 w-3" /> : <div className="h-2 w-2 rounded-full bg-current" />}
              </div>
              <div className="flex flex-col gap-1">
                <time className="text-xs font-medium text-white/56">{event.timestamp}</time>
